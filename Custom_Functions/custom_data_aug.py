...
def elastic_transform(image, alpha=2000, sigma=40, alpha_affine=40, random_state=None):
    ...
    for i in range(shape[2]):
        image[:, :, i] = cv2.warpAffine(
            image[:, :, i], M, shape_size[::-1], borderMode=cv2.BORDER_REFLECT_101
        )
    ...
    dx = (
        cv2.GaussianBlur(
            (random_state.rand(*shape_size) * 2 - 1),
    ...
    for i in range(shape[2]):
        def_img[:, :, i] = map_coordinates(image[:, :, i], indices, order=1).reshape(
            shape_size
        )
...
def salt_pepper_noise(image, salt=0.2, amount=0.004):
    ...
    for n in range(chan // 2):  # //2 so we don't augment the mask
        ...
        # Add Pepper noise
        coords = [
            np.random.randint(0, i - 1, int(num_pepper)) for i in image.shape[0:2]
        ]
