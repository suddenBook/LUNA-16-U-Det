...
def threshold_mask(raw_output, threshold):
    ...
    if len(props) >= 2:
        if (
            props[0].area / props[1].area > 5
        ):  # if the largest is way larger than the second largest
            thresholded_mask[all_labels == props[0].label] = (
                1  # only turn on the largest component
            )
        else:
            thresholded_mask[all_labels == props[0].label] = (
                1  # turn on two largest components
            )
            thresholded_mask[all_labels == props[1].label] = 1
    elif len(props):
        thresholded_mask[all_labels == props[0].label] = 1

    thresholded_mask = scipy.ndimage.morphology.binary_fill_holes(
        thresholded_mask
    ).astype(np.uint8)
...
def test(args, test_list, model_list, net_input_shape):
    if args.weights_path == "":
        weights_path = join(
            args.check_dir, args.output_name + "_model_" + args.time + ".hdf5"
        )
    else:
        weights_path = args.weights_path

    output_dir = join(
        args.data_root_dir, "results", args.net, "split_" + str(args.split_num)
    )
    ...
    with open(
        join(output_dir, args.save_prefix + outfile + "scores.csv"), "w"
    ) as csvfile:
        writer = csv.writer(
        ...
        for i, img in enumerate(tqdm(test_list)):
            img_data = np.load(
                join(args.data_root_dir, "imgs", "images_" + img[0])
            ).T  # 3d:(slices, 512, 512), 2d:(512, 512, channels=4)
            img_data = img_data[np.newaxis, :, :]
            sitk_img = sitk.GetImageFromArray(img_data)
            num_slices = img_data.shape[0]

            output_array = eval_model.predict(
            ...
            sitk.WriteImage(
                ...
                join(
                    raw_out_dir,
                    img[0][:-4] + "_raw_output" + img[0][-4:].replace(".npy", ".mhd"),
                ),
            )
            sitk.WriteImage(
                output_mask,
                join(
                    fin_out_dir,
                    img[0][:-4] + "_final_output" + img[0][-4:].replace(".npy", ".mhd"),
                ),
            )
            ...
            ax[0].imshow(
                output_bin[img_data.shape[0] // 3, :, :], alpha=0.5, cmap="Blues"
            )
            ax[0].imshow(gt_data[img_data.shape[0] // 3, :, :], alpha=0.2, cmap="Reds")
            ax[0].set_title(
                "Slice {}/{}".format(img_data.shape[0] // 3, img_data.shape[0])
            )
            ax[0].axis("off")

            ax[1].imshow(img_data[img_data.shape[0] // 2, :, :], alpha=1, cmap="gray")
            ax[1].imshow(
                output_bin[img_data.shape[0] // 2, :, :], alpha=0.5, cmap="Blues"
            )
            ax[1].imshow(gt_data[img_data.shape[0] // 2, :, :], alpha=0.2, cmap="Reds")
            ax[1].set_title(
                "Slice {}/{}".format(img_data.shape[0] // 2, img_data.shape[0])
            )
            ax[1].axis("off")

            ax[2].imshow(
                img_data[img_data.shape[0] // 2 + img_data.shape[0] // 4, :, :],
                alpha=1,
                cmap="gray",
            )
            ax[2].imshow(
                output_bin[img_data.shape[0] // 2 + img_data.shape[0] // 4, :, :],
                alpha=0.5,
                cmap="Blues",
            )
            ax[2].imshow(
                gt_data[img_data.shape[0] // 2 + img_data.shape[0] // 4, :, :],
                alpha=0.2,
                cmap="Reds",
            )
            ax[2].set_title(
                "Slice {}/{}".format(
                    img_data.shape[0] // 2 + img_data.shape[0] // 4, img_data.shape[0]
                )
            )
            ax[2].axis("off")
            ...
            plt.savefig(
                join(fig_out_dir, img[0][:-4] + "_qual_fig" + ".png"),
                format="png",
                bbox_inches="tight",
            )
            plt.close("all")
