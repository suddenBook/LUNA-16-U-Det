...
import os
os.chdir("C:/Users/huixi/OneDrive - Durham University/Final Project/Main_Code_Paper/0-Code_Data/")
...
def main(args):
    ...
    assert (
        args.train or args.test
    ), "Cannot have train, test all set to 0, Nothing to do."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", type=int, default=2, help="Batch size for training/testing."
    )
    parser.add_argument(
        "--initial_lr",
        type=float,
    ...
    parser.add_argument(
        "--save_prefix", type=str, default="", help="Prefix to append to saved CSV."
    )
    parser.add_argument(
        "--thresh_level",
        type=float,
    ...
    if arguments.which_gpus == -2:
        ...
        assert arguments.gpus != -1, (
            "Use all GPUs option selected under --which_gpus, with this option the user MUST "
